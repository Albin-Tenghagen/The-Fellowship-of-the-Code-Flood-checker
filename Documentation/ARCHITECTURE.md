...
Explain runtime view for different users (see [arc42 section 6](https://docs.arc42.org/section-6/)).

# 7. Deployment view

Explain deployment view (see [arc42 section 7](https://docs.arc42.org/section-7/)).

# 8. Concepts

Explain backend and frontend sections more indepth (see [arc42 section 8](https://docs.arc42.org/section-8/))

## Safety and Security

For safety the embedded system uses encrypted communication with backend using HTTPS. The system also uses a JWT and login to ensure only the sensors are allowed to send data to the backend. 

The MCUs has two ways to measure waterlevel so that in case one sensor breaks there will still exist good readings from the other sensor. 

## Operation and Administration

## Scaling

The hardware is easily scaleable because of its implementation. The code is divided up into sections that handle their respective tasks making it easy to add new tasks.
